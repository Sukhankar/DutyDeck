...
const Login = ({
  ...
  const fakeAuth = async (type) => {
    ...
    if (type === 'register') {
      ...
      handleUserRegister && handleUserRegister(email, password, name, organization)
    }
    if (type === 'login') {
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mt-6 gap-2">
          {view !== 'login' && (
            <button
              ...
              onClick={() => { setView('login'); setError(''); setPassword(''); setName(''); setOrganization(''); setConfirmPassword(''); }}
              type="button"
            >
              Back to Login
            </button>
          ...
          {view === 'login' && (
            <>
              <button
                className="text-blue-300 hover:underline text-xs"
                onClick={() => { setView('forgot'); setError(''); setPassword(''); setName(''); setOrganization(''); setConfirmPassword(''); }}
                type="button"
              >
                Forgot Password?
              </button>
              <button
                className="text-blue-300 hover:underline text-xs"
                onClick={() => { setView('register'); setError(''); setPassword(''); setName(''); setOrganization(''); setConfirmPassword(''); }}
                type="button"
              >
                Register
              </button>
