...
export default function RegisterMentor() {
  ...
  const [loadingStep, setLoadingStep] = useState(""); // "otp", "verify", "register", "resend"
  ...
  const sendOtp = async () => {
    setLoadingStep("otp");
    try {
      await API.post("/auth/send-mentor-otp", { email: form.email });
      setOtpSent(true);
  ...
  const verifyOtp = async () => {
    ...
    try {
      await API.post("/auth/verify-mentor-otp", { email: form.email, otp: enteredOtp });
      setOtpVerified(true);
